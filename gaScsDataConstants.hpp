...
// axis index enumeration
  ...
  // Some routines may use tis enumeration to iterate through a vector so be careful about changing it
  enum AxisIndexes {
    AXIDX_UNKNOWN= 0,
    AXIDX_A_FT_IN= 1,
    AXIDX_A_FT_OUT,
...
// coil and RIA features mostly used for position calculation
  ...
  const double DEG_TO_RADIANS= PI/180;  // convert between degrees and radians for trig functions
  const double RADIANS_TO_DEG= 180/PI;
  
  const long TURNS_PER_LAYER= 14;
  const long LAYERS_PER_COIL= 40;
//  const long COIL_ANGLE_MAX= 61000; // test value
  ...
  const double SOC_POST_LOAD_ANGLE= -140;
  const double SOC_INIT_RETR_ANGLE= -130;
  const double SOC_INIT_ADV_ANGLE= -80;
  ...
  const double JOGGLE_RETRACT_ADJ_THRESHOLD= 360; // number of degrees prior to a joggle that a retract adjustment is made
  const double JOGGLE_FULL_RETRACT_THRESHOLD= 0; // number of degrees prior to (or after) a joggle that a full retract is made
  const double JOGGLE_ADV_TO_FIRST_THRESHOLD= -360; // number of degrees past a joggle that the advancing foot moves
...
// Event related values
  ...
  // RIA features
  const long OFFSET_RET_FOOT= -1 * abs(RET_FOOT_RIA_OFFSET_ANGLE); // make negative
  const long OFFSET_PLOW= -55;  // TODO: confirm
  const long OFFSET_ADV_FOOT= -1 * abs(ADV_FOOT_RIA_OFFSET_ANGLE); // make negative
...
//  const std::string DB_SERVER_NAME= "10.6.1.10";
  ...
  const std::string SPNAME_SELECT_HQPSTART_ANGLES= "coil.sprocSelectStartHqpAngles"; // Coil Map Hqp start angles
