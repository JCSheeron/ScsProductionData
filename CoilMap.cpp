...
CoilMap::CoilMap() :
  ...
  long len = sizeof(LA_ME_CO) / sizeof(LA_ME_CO[0]);   // get the number of elements
  laMeCo_.assign(LA_ME_CO, LA_ME_CO + len); // assign the array elements to the member vector
...
long CoilMap::PopulateCoilMap() {
  ...
  if (RTN_NO_ERROR == connectStatus && RTN_NO_ERROR == queryStatus) {
    opStatus= MapFeature();
    }
  ...
  // if connect status, previous query, and previous operation were all okay,
  // continue and query the db to get the hqp start angles
  if (RTN_NO_ERROR == connectStatus &&
      RTN_NO_ERROR == queryStatus &&
      RTN_NO_ERROR == opStatus) {
  ...
  if (RTN_NO_ERROR == connectStatus &&
      RTN_NO_ERROR == queryStatus &&
      RTN_NO_ERROR == opStatus) {
    opStatus= PopulateHqpStartAngleSet();
    }
...
CoilMap::fhltar CoilMap::GetFhltarLb(double angle) const {
  CoilMap::cm_cit cit= mapCoil_.upper_bound(angle);  // Get the position at or past the specified angle
...
double CoilMap::GetHqpStartLb(double angle) const {
  // return value indicates success or error
  // the set being looked in contains only hqp start angles
  CoilMap::as_cit cit= setHqpStartAngles_.upper_bound(angle);  // Get the position past the specified angle
  if (cit != setHqpStartAngles_.begin()) // if not at the beginning
    return *(--cit);  // decrement the iterator and return the start angle
  else
    return RTN_NO_RESULTS;
  }
...
/*
// determine if a new HQP is about to start
// In general, this is signaled when the current LB FC is a Joggle, and the next FC is a Local Zero (L).
// The very first hqp (the beginning of the coil) is signaled with a FC of a local zero (L)
// and an LB angle of 0 (beginning of the coil map).
bool CoilMap::isNewHqpLb(double angle, bool &condition) const {
  // Get the current Lb feature code and the one after that
  fc_pair curNextFc= GetCurrentNextFcLb(angle); 
  // Get the current LB angle
  double curAngle= GetAngleLb(angle);
  // look for feature code J followed by L
  if ((FC_JOGGLE == curNextFc.first && FC_LOCAL == curNextFc.second) || // general case, J followed by L
      (FC_LOCAL == curNextFc.first && curAngle == 0.0)) {               // start of coil case, L at angle 0.
    // this is the condition for a new Hqp. 
    condition= true;
    return true;
    }
  else if ((NO_FEATURE_STR != curNextFc.first && NO_FEATURE_STR != curNextFc.second && NO_FEATURE != curAngle) && // lookups worked
           !(FC_JOGGLE == curNextFc.first && FC_LOCAL == curNextFc.second) && // not the general case
           !(FC_LOCAL == curNextFc.first && curAngle == 0.0)) {               // not the start of coil case
    // a feature and next feature is found for the current angle, but
    // they don't satisfy the condition for a new HQP
    condition= false;
    return true;
    }
  else { // error case, or no features found
      condition = false;
      return false;
    }
  }
  
// Determine if a new layer is about to start.
// This is signaled when:
// The current angle is within a column increment of the next joggle
  // Next Joggle Angle - current angle < Column Increment amount
// OR 
// when the current angle is within the joggle window of the previous joggle
// AND 
// when the feature code after the joggle is not an Local Zero (L), which is the new HQP case
  // (Previous Joggle Angle + Joggle window) - current angle >= 0
  // Note that the joggle window is even/odd layer dependent. Odd to even layer
  // joggles are on turn 14, and even to odd layer joggles are on turn 1.
bool CoilMap::isNewLayerLb(double angle, bool &condition) const {
  ...
  // TODO: Any need to specifically handle error case when searches return no feature??
  if (((RTN_NO_RESULTS != nextJoggleAngle && nextJoggleAngle - angle < COLUMN_INCREMENT) || // within an increment of an upcoming joggle
      (RTN_NO_RESULTS != prevJoggleAngle && (prevJoggleAngle + prevJoggleLength) - angle >= 0)) && // within the joggle length of a past joggle
      !(FC_JOGGLE == curNextFc.first && FC_LOCAL == curNextFc.second)) { // and not the start of an hqp (fc J followed by L)
    // Joggle is the Lb, and the next fc is not a local zero
    // distance before or after a joggle puts us near enough ...
    // ... this is the condition for a new layer. 
    condition= true;
    return true;
    }
  else {
...
// Determine if the angle (column) is the last move (retreat) of the layer, 
  // and return by reference the boolean condiiton of the joggle angle. If the 
  // test for the last move (the condition) is false, then the angle is unchanged.
// This is signalled when:
...
bool CoilMap::isLastMoveOfLayer(double angle, bool &condition, double &joggleAngle) const {
...
bool CoilMap::isInLaMeCo(long layer) const {
 ...
 cit = find(laMeCo_.begin(), laMeCo_.end(), layer); // look for the value
...
long CoilMap::QueryDb(const std::string &sprocName) {
  ...
  // query the db using a SELECT command to fetch the coil map data
...
long CoilMap::PopulateHqpStartAngleSet() {
  // Populates a set of hqp start angles sorted ascendingly
  // return value indicates success or error

  // QueryDb() is called with the correct stored procedure name prior to this function being called.
  // The query returns a sorted list of angles which correspond to hqp start angles
  // Put the values fetched by the QueryDb() into a set
  // QueryDb() should return one or more rows.  Map every row returned.
  ...
  try {
    if (dbCommand_.isResultSet() ) {
      ...
      while(dbCommand_.FetchNext() ) {
        // get angle
        // add the layer and associated angle to the map
        setHqpStartAngles_.insert(dbCommand_.Field(CM_ANGLE_PARAM.c_str()).asDouble());  // insert the retreived angle into the set
        }
      // set return value for all okay
      rtnValue= RTN_NO_ERROR;
      }
