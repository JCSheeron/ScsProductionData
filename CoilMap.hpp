...
public:
  ...
  // Define a map to contain a list of layers (key) and coil angles, but only including odd layer turn 14 Transitions.
    ...
    // the coil map is implemented as a map of <long, angle>
      typedef boost::container::map<long, double> layerAngle_map;
      typedef layerAngle_map::const_iterator lam_cit;

  // Define a vector to hold the layer numbers corresponding when to perform a layer compression and measurement
      typedef std::vector<long> layer_container;
      typedef layer_container::const_iterator lc_cit;
      
  // Define a set to contain a list of angles.
    typedef boost::container::set<double> angle_set;
 ...
 /*    
    // returned value indicates an angle was found, passed in reference indicates the condition being tested.
    // return will be false if map is empty or if angle is before (smaller than) the first entry
    // determine if a new HQP is about to start
    // this is signaled when the current LB FC is a Joggle, and the next FC is a Local Zero (L)
    bool isNewHqpLb(double angle, bool &condition) const;
    // determine if a new layer is about to start
    // this is signaled when the next layer number is greater than the current layer number
    bool isNewLayerLb(double angle, bool &condition) const;
*/
    // Determine if the angle (column) is the last move (retreat) of the layer, 
      // and return by reference the boolean condiiton and the joggle angle.
  ...
  private:
    // helper functions
      ...
      long PopulateHqpStartAngleSet();   // Populates a set of hqp start angles sorted ascendingly
    ...
    // member variables
      ...
      // Set of hqp start angles. Given and angle, this set allows the lookup of the angle of the next or previous hqp.
      angle_set setHqpStartAngles_;
      // list of layer numbers when coil measurement and compression are needed
      layer_container laMeCo_;
      // previous hqp value -- used to detect a new hqp
      double hqpStartPrev_; 
