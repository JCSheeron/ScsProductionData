...
// private helper functions
  ...
  double AxisPositions::CalculateTransitionAdjustment(double angle) {
    ...
    if (transStat_ && transResult_) {  // sucessful lookup and odd layer
      ...
      // Calculate large (r2) adn small (r1) radius and RArc
  ...
  void AxisPositions::ClearAllTransAdjust() {
    for (long column = 0; column < arrAdjMark_.size(); ++ column)
  ...
  AxisPositions::joggleAdjustmentType AxisPositions::CalculateJoggleAdjustmentType(double angle, double &degToNextJoggle, double &degToPrevJoggle, double &jAdj) const {
    ...
    if ((JOGGLE_RETRACT_ADJ_THRESHOLD < degToNextJoggle) && // next is too far ahead for region 1
      ...
      // joggle adjustment not used for this type of joggle adjustment
      jAdj= 0;
      // adjustment type is to not move the retreating foot, and do a nominal index of the advancing foot
      return JA_RET_NOP_ADV_NOM;
      }
  ...
  double AxisPositions::CalculateLoadHexRiaAngle(double angle) const {
    return 0.0;
    }
  ...
  AxisPositions::angleIsEven_pair AxisPositions::CalculateNewLayerRiaAngle(double coilAngle, double joggleAngle) const {
    ...
    return std::make_pair(riaAngle, isEven);
      ...
      // If insert mode is IM_ABS_ALL or IM_REL_ALL, then move the feet inside axes to pos 1 and outside axes to pos 2.
        ...
        // The foot role parameter is not used (has one effect) in the IM_ABS_ALL or IM_REL_ALL insert modes.
  ...
  long AxisPositions::PopScsPosDetail(double coilAngle, bool isEven, footRole role, insertMode mode,
    ...
    if (IM_ABS_ALL == mode || IM_REL_ALL == mode) {  // either of the insert all modes
      // absolute and relative act the same, except for the position detail.absolute flag 
      // set all the inner feet to pos 1
      ...
      // Foot set all the outer feet to pos 2
      ...
      // set the, trace, isAbsolute, isTransition and isJoggleAdj flags and return. 
      posDetail.get<4>().get<0>()= trace;  // logic trace string
      // set the absolute flag depending on the mode
  ...
  void AxisPositions::MapPostLoadScsPositions(double angle) {
    ...
    // Look up the Joggle angle
    double newHqpAngle= coilMap_.GetAngleLb(angle);
    // make a place to store a trace string
    std::string trace;
    
    if (NO_FEATURE != newHqpAngle) {
    ...
    PopScsPosDetail(angle, false, FOOT_ROLE_ADVANCING, IM_ABS_ALL,  // angle, isEven, foot role, insert mode
                    ...
                    false, false, posDetail);         // isLastTurn, isLastLayer, posDetail
    // add to the map
  ...
  void AxisPositions::MapCoilStartScsPositions(double socPostLoadAngle, double socInitRetrAngle, double socInitAdvAngle, double &tAdj) {
    ...
    posDetail.get<2>().assign((COLUMN_COUNT * 2) + 1, false); // selected axis vector. make space for each axis plus 1 for the flag, and init to false.
      ...
      PopScsPosDetail(((F_COLUMN_AZIMUTH - 10) - 360.0), false, FOOT_ROLE_ADVANCING, IM_ABS_ALL,  // coil angle, isEven, foot role (does not matter), insert mode
                      RETREATING_FOOT_START_POS, ADVANCING_FOOT_START_POS,  // inner feet to posDist 1, outer feet to posDist 2
                      trace, false, false, true, false, // trace, isTransition, isjoggle, isNewHqp, isNewLayer
                      false, false, posDetail);         // isLastTurn, isLastLayer, posDetail
    ...
    if (stat && result) { // in a transition window -- this should always be true for the beginning of the coil
      ...
      PopScsPosDetail((F_COLUMN_AZIMUTH - 360.0), false, FOOT_ROLE_RETREATING, IM_ABS_UPDATE_SEL,  // coil angle, isEven, foot role, mode (absolute update select)
      ...
      PopScsPosDetail((F_COLUMN_AZIMUTH - 360.0), false, FOOT_ROLE_ADVANCING, IM_ABS_UPDATE_SEL,  // coil angle, isEven, foot role, mode (absolute update select)
    ...
    // Enter the new layer foot positions into the postion map at the specified angle.
      // Used to "seed" the starting point for relative adjustments to absolute positons later on.
  void AxisPositions::MapNewLayerScsPositions(double riaAngle, double coilAngle, bool isEven) {
    ...
    if (isEven) {
      ...
      trace= "New Even Layer Start Positions. Column Angle: " + std::to_string(static_cast<long long>(coilAngle)) + ". Inner feet to adv. start, Outer feet to retr. start. Columns not known, use sentinel.";
      PopScsPosDetail(coilAngle, isEven, FOOT_ROLE_ADVANCING, IM_ABS_ALL,  // angle, isEven, foot role, insert mode
                      ADVANCING_FOOT_START_POS, RETREATING_FOOT_START_POS,  // inner feet to posDist 1, outer feet to posDist 2
                      trace, false, false, false, true, // trace, isTransition, isjoggle, isNewHqp, isNewLayer
                      false, false, posDetail);         // isLastTurn, isLastLayer, posDetail
      }
    else { 
    ...
    } // AxisPositions::MapNewLayerScsPositions(double riaAngle, double coilAngle, bool isEven)
  ...
  void AxisPositions::CalculateAxisMoves() {
    ...
    long layerNumber;
    long layerNumberPrev;
    long hqpNumber;
    long hqpNumberPrev;
/*
    // New hqp (hex/quad pancake) reference set (absolute positions entered, post load mode).
    // Flag signals a new HQP row was entered, and not to enter another one, and not to enter 
    // a "new layer row"  until the condition is detected again and the flag is clear
    bool isNewHqpRefSet= false;

    // New layer reference set (absolute positions entered to start feet a "start positions") for new layer.
    // Flag signals a layer row was entered, and not to enter another one until the condition is detected again
    // and the flag is clear
    bool isNewLayerRefSet= false;
*/
    // even/odd layer flag
    ...
    for (long currentAngle= INITIAL_COLUMN_ANGLE; currentAngle <= COIL_ANGLE_MAX; currentAngle += COLUMN_INCREMENT) {
      ...
      if (currentAngle == INITIAL_COLUMN_ANGLE) {
        ...
        stat= coilMap_.isLastMoveOfLayer(currentAngle, result, joggleAngle);
        if (stat && result) {
          ...
          // joggle, but the hex/quad number only changes at a new hex -- check for hex first so it keeps from also checking new lsyer
          ...
          if (hqpNumber != hqpNumberPrev) {
            ...
            MapPostLoadScsPositions(currentAngle);
            ...
            MapNewLayerScsPositions(prAie.first, currentAngle, prAie.second);
...
/*      
      stat= coilMap_.isNewHqpLb(currentAngle, result);
      if (stat && result && !isNewHqpRefSet) { // new Hqp is about to start, and reference flag not set
        // Enter an absolute position for all the feet in the scs position map.
        // The position entered should be the nominal starting locations after
        // load mode is completed. The Position Not Caluclated value is used 
        // for the column positions (these are not known until run time).
        
        // The reference flag is used so only 1 row is written (not a row at every column azimuth)
        MapPostLoadScsPositions(currentAngle);
        // set the reference set flag
        isNewHqpRefSet= true;
//        // set the new layer reference flag so we don't get a new hex and a new layer
//        isNewLayerRefSet= true;
        }
      else if (stat && result && isNewHqpRefSet) {
        // new HQP starting, but reference is set
        // An entry was already made -- do nothing until the  flag 
        // is cleared and another HQP is starting
        }
      else if (stat && !result) {
        // new HQP not starting
        // reset the set reference flag
        isNewHqpRefSet= false;
        }
      else { // error case - should never get here
        logicTrace.append("Error looking for new HQP @ angle: " + std::to_string(static_cast<long long>(currentAngle)) + ". ");
        // reset the set reference flag
        isNewHqpRefSet= false;
      }

      // See if a new layer about to start.
      stat = coilMap_.isNewLayerLb(currentAngle, result);
      if (stat && result && !isNewLayerRefSet) { // new Layer is about to start, and reference flag not set
          // Enter an absolute position for all the feet in the scs position map.
          // The position entered should be the nominal starting locations.
          // The Position Not Calculated value is used for the column
          // positions (these are not known until run time).
          ...
          // Determine the ria angle where the starting positions should go
          angleIsEven_pair prAie = CalculateNewLayerRiaAngle(currentAngle); // prAie -- Angle Is Even pair
          
          // Create and map the resulting position detail
          MapNewLayerScsPositions(prAie.first, currentAngle, prAie.second);
          }
        else if (stat && result && isNewLayerRefSet) {
          // new layer starting, but reference is set
          // An entry was already made -- do nothing until the flag 
          // is cleared and another layer is starting
          }
        else if (stat && !result) {
          // new layer not starting
          // reset the set reference flag
          isNewLayerRefSet= false;
          }
        else { // error case - should never get here
          logicTrace.append("Error looking for new layer start @ angle: " + std::to_string(static_cast<long long>(currentAngle)) + ". ");
          // reset the set reference flag
          isNewLayerRefSet= false;
        }
...
*/        
      ...
      long riaAngle= POSITION_NOT_CALCULATED; // sentinel values. Should not remain with this value
      double FootPosDist= POSITION_NOT_CALCULATED;
      if (!isLastLayer) {
        // not the last layer
        // Capture RIA angle for advancing feet
        riaAngle= currentAngle - ADV_FOOT_RIA_OFFSET_ANGLE;
        ...
        PopScsPosDetail(currentAngle, isEvenLayer, FOOT_ROLE_ADVANCING, scsInsertMode, 
                        FootPosDist, 0, logicTrace, inTransition, isJoggleAdj, false, false, 
                        isLastTurn, isLastLayer, scsPosDetail_);
        ...
        // create entry in SCS position map
      ...
      riaAngle= currentAngle - RET_FOOT_RIA_OFFSET_ANGLE;
      ...
      // populate position detail structure
      // isNewHQP and isNewLayer cases are handled elsewhere, so these are forced to false here
      PopScsPosDetail(currentAngle, isEvenLayer, FOOT_ROLE_RETREATING, scsInsertMode,
                      FootPosDist, 0, logicTrace, inTransition, isJoggleAdj, false, false,
                      isLastTurn, isLastLayer, scsPosDetail_);
